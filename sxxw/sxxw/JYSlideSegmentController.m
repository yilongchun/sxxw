...
//      //设置边框线的颜色
//      [layer setBorderColor:[[UIColor colorWithRed:220/255.0 green:220/255.0 blue:220/255.0 alpha:1] CGColor]];
  }
  return self;
}
  ...
  [self reset];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(loadData)
                                                 name:@"reloadSegmentBar"
                                               object:nil];
}

-(void)loadData{
    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:_selectedIndex inSection:0];
    JYSegmentBarItem *segmentBarItem = [self.segmentBar dequeueReusableCellWithReuseIdentifier:segmentBarItemID
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  segmentBarItem.titleLabel.text = vc.title;
    segmentBarItem.titleLabel.adjustsFontSizeToFitWidth = YES;
    segmentBarItem.titleLabel.minimumScaleFactor = 0.5f;
    if (indexPath.row == _selectedIndex) {
        segmentBarItem.titleLabel.textColor = [UIColor colorWithRed:72/255.0 green:147/255.0 blue:219/255.0 alpha:1];
    }else{
